...
dials = serial.Serial('/dev/cu.PL2303-0030134', 119200, timeout=1, parity=serial.PARITY_ODD, bytesize=8)
...
try:
  while True:
    ...
    if value:
      ...
      if (test > 0):
        dialsCounter [dialNo] -= abs((248-position)-(248-_position))
      elif (test < 1 and direction > 0):
        dialsCounter [dialNo] -= 1
      else:
        dialsCounter [dialNo] += abs(position-_position);
